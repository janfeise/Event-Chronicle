...
function parseResponse(response, params) {
  ...
  const parsed = Object.assign({}, response, { output: output2 });
  ...
  [(_ResponseStream_params = /* @__PURE__ */ new WeakMap(), _ResponseStream_currentResponseSnapshot = /* @__PURE__ */ new WeakMap(), _ResponseStream_finalResponse = /* @__PURE__ */ new WeakMap(), _ResponseStream_instances = /* @__PURE__ */ new WeakSet(), _ResponseStream_beginRequest = function _ResponseStream_beginRequest2() {
    ...
    switch (event.type) {
      case "response.output_text.delta": {
        const output2 = response.output[event.output_index];
        if (!output2) {
          throw new OpenAIError(`missing output at index ${event.output_index}`);
        }
        if (output2.type === "message") {
          const content = output2.content[event.content_index];
          if (!content) {
    ...
    switch (event.type) {
      ...
      case "response.content_part.added": {
        const output2 = snapshot.output[event.output_index];
        if (!output2) {
          throw new OpenAIError(`missing output at index ${event.output_index}`);
        }
        const type = output2.type;
        const part = event.part;
        if (type === "message" && part.type !== "reasoning_text") {
          output2.content.push(part);
...
function detectPromptsDir() {
  const prodCandidate = path3.resolve(__dirname, "prompts");
...
var PromptManager = class {
  ...
  reinitialize(promptsDir) {
    this.promptsDir = path3.resolve(promptsDir);
    this.cache.clear();
  }
  /**
   ...
   */
  resolvePath(name) {
    return path3.join(this.promptsDir, `${name}.md`);
  }
  /**
...
var _dataDir = path4.resolve(process.cwd(), "data");
function getDataDir() {
  return _dataDir;
}
function setDataDir(dir) {
  _dataDir = path4.resolve(dir);
}
...
function writeToFile(line) {
  if (!logToFile) return;
  try {
    const dir = path5.resolve(getDataDir(), "logs");
    if (!fs2.existsSync(dir)) {
      fs2.mkdirSync(dir, { recursive: true });
    }
    const date = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
    const filePath = path5.join(dir, `${date}.log`);
...
function loadChronicle(existingEventId) {
  ...
  const filePath = path6.join(dataDir, `${existingEventId}.json`);
...
function saveChronicle(events) {
  ...
  const filePath = path7.join(dataDir, filename);
...
function appendEvents(newEvents, eventId) {
  ...
  if (resolvedId) {
    ...
    const filePath2 = path8.join(dataDir, filename2);
...
function stateFilePath(eventId) {
  return path9.join(getDataDir(), `${eventId}_state.json`);
}
