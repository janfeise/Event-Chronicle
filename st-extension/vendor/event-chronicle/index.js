...
function detectPromptsDir() {
  const prodCandidate = path2.resolve(__dirname, "prompts");
...
var PromptManager = class {
  ...
  reinitialize(promptsDir) {
    this.promptsDir = path2.resolve(promptsDir);
    this.cache.clear();
  }
  /**
   ...
   */
  resolvePath(name) {
    return path2.join(this.promptsDir, `${name}.md`);
  }
  /**
...
var _dataDir = path3.resolve(process.cwd(), "data");
function getDataDir() {
  return _dataDir;
}
function setDataDir(dir) {
  _dataDir = path3.resolve(dir);
}
...
function writeToFile(line) {
  if (!logToFile) return;
  try {
    const dir = path4.resolve(getDataDir(), "logs");
    if (!fs2.existsSync(dir)) {
      fs2.mkdirSync(dir, { recursive: true });
    }
    const date = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
    const filePath = path4.join(dir, `${date}.log`);
...
function loadChronicle(existingEventId) {
  ...
  const filePath = path5.join(dataDir, `${existingEventId}.json`);
...
function appendEvents(newEvents, eventId) {
  ...
  if (resolvedId) {
    ...
    const filePath2 = path7.join(dataDir, filename2);
...
function stateFilePath(eventId) {
  return path8.join(getDataDir(), `${eventId}_state.json`);
}
